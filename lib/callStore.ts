...
export const useCallStore = create<CallState>((set, get) => ({
  ...
  startCall: async (targetUserId, name, avatar) => {
    ...
    try {
      ...
      let _iceDisconnectTimer: ReturnType<typeof setTimeout> | null = null;
      (pc as any).oniceconnectionstatechange = () => {
        ...
        if (iceState === "connected" || iceState === "completed") {
          if (_iceDisconnectTimer) {
            ...
            console.log("[CallStore] ICE reconnected — disconnect timer cleared");
          }
        ...
        // Temporary disconnect — grace period before giving up
          ...
          console.log("[CallStore] CALLER ICE temporarily disconnected — starting 15s grace period");
          _iceDisconnectTimer = setTimeout(() => {
            if ((pc as any).iceConnectionState === "disconnected") {
              console.log("[CallStore] CALLER ICE disconnect timeout exceeded — cleaning up");
              get()._cleanup();
            }
          }, 15_000);
        }

        // Unrecoverable states only
        else if (iceState === "failed" || iceState === "closed") {
          console.log("[CallStore] CALLER ICE failed/closed — cleaning up");
    ...
    try {
      ...
      (pc as any).oniceconnectionstatechange = () => {
        ...
        // Temporary disconnect — grace period before giving up
          ...
          console.log("[CallStore] RECEIVER ICE temporarily disconnected — starting 15s grace period");
          _iceDisconnectTimer = setTimeout(() => {
            if ((pc as any).iceConnectionState === "disconnected") {
              console.log("[CallStore] RECEIVER ICE disconnect timeout exceeded — cleaning up");
              get()._cleanup();
            }
          }, 15_000);
        }
        ...
        // Unrecoverable states only
          ...
          console.log("[CallStore] RECEIVER ICE failed/closed — cleaning up");
          if (_iceDisconnectTimer) clearTimeout(_iceDisconnectTimer);
          get()._cleanup();
        }
      };
  ...
  _transitionToActive: () => {
    ...
    // IMPORTANT: Do NOT call setMicrophoneMute here — it must only
    // be changed by the user via toggleMute().
    try {
      InCallManager.setKeepScreenOn(true);
      InCallManager.setSpeakerphoneOn(true);
      console.log("[CallStore] InCallManager audio routing set");
    } catch (e) {
      console.warn("[CallStore] InCallManager routing failed:", e);
    }

    // Start track monitor for debugging
    const trackMonitorId = setInterval(() => {
      const { localStream: ls } = get();
      ls?.getAudioTracks().forEach((track: any) => {
        console.log(
          "[Track Monitor]",
          track.id,
    ...
    // Force-enable all local audio tracks on transition
    // to ensure they are never accidentally muted during call setup
    const { localStream: ls } = get();
    ls?.getAudioTracks().forEach((track: any) => {
      if (!get().isMuted) {
        // Only force-enable if the user hasn't pressed mute
        track.enabled = true;
        console.log("[CallStore] Track enabled on active transition:", track.id);
      }
    });
  ...
  toggleMute: () => {
    ...
    // ONLY place in the entire codebase that should touch track.enabled
    state.localStream?.getAudioTracks().forEach((track: any) => {
      track.enabled = !newMuted; // enabled=true means NOT muted
      console.log(
        "[Mute Toggle]",
        track.id,
        "enabled=",
    ...
    // Also sync InCallManager mic mute state
    try {
      InCallManager.setMicrophoneMute(newMuted);
    } catch (e) {
      console.warn("[CallStore] setMicrophoneMute failed:", e);
    }
